...
class PatchEmbedding(nn.Module):
    """Convierte una imagen 2D en una secuencia de embeddings de parches lineales."""

    def __init__(self, img_size=224, patch_size=16, in_channels=1, embed_dim=768) -> None:
        super().__init__()
...
class MultiHeadAttention(nn.Module):
    """Multi-head Attention para Vision Transformer."""
...
class ViT_2D(nn.Module):
    """Vision Transformer para imágenes médicas 2D."""
    ...
    def forward(self, x):
        # Asegurarse que la imagen tiene el tamaño correcto
        _, _, H, W = x.shape
        if self.patch_embed.img_size != H or self.patch_embed.img_size != W:
            x = F.interpolate(
                x,
