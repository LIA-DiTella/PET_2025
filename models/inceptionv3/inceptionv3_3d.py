...
class InceptionV3_3D(nn.Module):
    ...
    def forward(self, x):
        ...
        # Asegurar que la entrada tenga las dimensiones mínimas
        # I3D funciona mejor con al menos 16 frames temporales
        if x.size(2) < 16:  # Si depth < 16
            # Repetir frames para llegar al mínimo
            repeat_factor = (16 + x.size(2) - 1) // x.size(2)  # Ceil division
            x = x.repeat(1, 1, repeat_factor, 1, 1)[:, :, :16]  # Tomar solo los primeros 16
